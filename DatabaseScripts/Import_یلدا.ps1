# =============================================
# Script PowerShell: Import فایل یلدا.xlsx
# Description: این اسکریپت فایل "یلدا.xlsx" را از Desktop می‌خواند و import می‌کند
# =============================================

param(
    [Parameter(Mandatory=$false)]
    [string]$ExcelFilePath = "$env:USERPROFILE\Desktop\یلدا.xlsx",
...
Write-Host "Import فایل یلدا.xlsx" -ForegroundColor Green
Write-Host "=========================================" -ForegroundColor Green
...
if (-not (Test-Path $ExcelFilePath)) {
    ...
    Write-Host "لطفاً مطمئن شوید فایل 'یلدا.xlsx' در Desktop قرار دارد." -ForegroundColor Red
    exit 1
}
...
if (-not (Get-Module -ListAvailable -Name ImportExcel)) {
    ...
    try {
        ...
        Write-Error "خطا در نصب ماژول ImportExcel: $_"
        Write-Host "لطفاً دستور زیر را به صورت Administrator اجرا کنید:" -ForegroundColor Yellow
...
try {
    ...
    Write-Host "ممکن است نام Sheet متفاوت باشد. لطفاً نام Sheet را با پارامتر -SheetName مشخص کنید." -ForegroundColor Yellow
    exit 1
}
...
foreach ($row in $excelData) {
    try {
        ...
        $checkParam = $checkCommand.Parameters.Add("@NationalCode", [System.Data.SqlDbType]::NVarChar, 10)
