...
namespace Shahkar.Provider
{
    public class ShahkarService(IHttpClientFactory http, IOptions<UidApiOptions> options, ILogger<ShahkarService> logger) : IShahkarService
    {
        ...
        private readonly ILogger<ShahkarService> _logger = logger;
        ...
        //    // افزودن User-Agent header که ممکن است توسط سرویس UID مورد نیاز باشد
        //    message.Headers.Add("User-Agent", "LoanGateway-Auth/1.0");

        //    message.Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json");

        //    _logger.LogInformation(
        //        "در حال ارسال درخواست به سرویس Shahkar برای استعلام اطلاعات هویتی. NationalId: {NationalId}, BirthDate: {BirthDate}, Url: {Url}",
        //        nationalId, birthDate, url);
        ...
        //    _logger.LogInformation(
        //        "پاسخ دریافت شده از سرویس Shahkar. StatusCode: {StatusCode}, ResponseBody: {ResponseBody}",
        //        response.StatusCode, responseBody);

        //    if (!response.IsSuccessStatusCode)
        //    {
        //        _logger.LogError(
        //            "خطا در ارتباط با سرویس Shahkar. StatusCode: {StatusCode}, ResponseBody: {ResponseBody}",
        //            response.StatusCode, responseBody);
        ...
        //    {
        //        _logger.LogError(
        //            "ناتوانی در deserialize پاسخ سرویس Shahkar. ResponseBody: {ResponseBody}",
        //            responseBody);
        ...
        //    _logger.LogInformation(
        //        "وضعیت پاسخ سرویس Shahkar. StatusCode: {StatusCode}, Message: {Message}, Code: {Code}",
        //        response.StatusCode, status.message, status.code);

        //    if (status.code != 0)
        //    {
        //        _logger.LogWarning(
        //            "استعلام اطلاعات هویتی ناموفق بود. StatusCode: {StatusCode}, Message: {Message}, Code: {Code}, FullResponse: {ResponseBody}",
        //            response.StatusCode, status.message, status.code, responseBody);
        ...
        public async Task<ShahkarGetPersonInfoResponseDto> GetPersonalInfoAsync(string nationalId, string birthDate, CancellationToken cancellationToken = default)
        {
            var normalizedBirthDate = ConvertPersianDigitsToEnglish(birthDate);
            var normalizedNationalId = ConvertPersianDigitsToEnglish(nationalId);
            ...
            _logger.LogInformation(
                "در حال ارسال درخواست به سرویس Shahkar برای استعلام اطلاعات هویتی. NationalId: {NationalId} -> {NormalizedNationalId}, BirthDate: {BirthDate} -> {NormalizedBirthDate}, Url: {Url}",
                nationalId, normalizedNationalId, birthDate, normalizedBirthDate, url);
        ...
        public async Task<ShahkarGetAddressUserResponseDto> GetAddressUserAsync(string postalcode, CancellationToken cancellationToken = default)
        {
            var client = _http.CreateClient();
            var req = new
            {
            ...
            var url = _options.Value.BaseUrl + "/api/inquiry/address/v2";
            ...
            if (!response.IsSuccessStatusCode)
            {
                throw new ExternalServiceException(
                    "UID-Address",
                    response.StatusCode,
                    "خطا در تماس با سرویس personal-Address.",
                    AppErrorCodes.ExternalServiceError);
            }

            var result = JsonSerializer.Deserialize<ShahkarGetAddressUserResponseDto>(
                responseBody, _json);
            ...
            if (result is null)
            {
                throw new ExternalServiceException(
                    "UID-Address",
                    HttpStatusCode.BadGateway,
                    "پاسخ نامعتبر از سرویس personal-Address.",
                    AppErrorCodes.ExternalServiceError);
            }
            var status = result.responseContext.status;
            if (status.code != 0)
            ...
            return new ShahkarGetAddressUserResponseDto
            {
                address = result.address,
                postalCode = result.postalCode,
                responseContext = new ShahkarGetAddressUserResponseDto.ResponseContext
                {
                    correlationId = result.responseContext.correlationId,
                    custom = result.responseContext.custom,
                    status = result.responseContext.status,
