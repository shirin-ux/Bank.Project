...
namespace LoanService.Domain.Entities.Investment
{
    public class InvestmentAccount : BaseEntity
    {
        ...
        private InvestmentAccount(  Guid id, string agentId,long policyId,string nationalCode,DateTime birthDate,string planTypeAliasName, string? postalCode,string? address, string traceId)
        {
            Id = id;
            AgentId = agentId;
            PolicyId = policyId;
            ...
            BirthDate = birthDate;
            PlanTypeAliasName = planTypeAliasName;
            PostalCode = postalCode;
            Address = address;

            State = InvestmentState.New;
        ...
        public string PlanTypeAliasName { get; private set; } = default!; // Gold, Silver, FixedIncome, ...
            ...
            string traceId)
        {
            ...
            if (string.IsNullOrWhiteSpace(planTypeAliasName))
                throw new LogicException("نوع طرح سرمایه‌گذاری (planTypeAliasName) الزامی است.");

            if (string.IsNullOrWhiteSpace(traceId))
                throw new LogicException("TraceId برای ایجاد حساب الزامی است.");
